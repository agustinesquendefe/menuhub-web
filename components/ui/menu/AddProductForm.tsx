  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="space-y-3 bg-white p-3 rounded-lg border border-dashed">
      ...
      <div className="grid grid-cols-2 gap-5">
        <div>
          <Label htmlFor={`prod-name-${categoryId}`} className="text-sm">Name</Label>
          <Input
        ...
        <div>
          <Label htmlFor={`prod-price-${categoryId}`} className="text-sm">Price</Label>
          <Input
      ...
      <div>
        <Label htmlFor={`prod-desc-${categoryId}`} className="text-sm my-5">Description (optional)</Label>
        <Input
      ...
      <div>
        <Label htmlFor={`prod-image-${categoryId}`} className="text-sm my-5">Image</Label>
        <Input
      ...
      <div className="flex items-center gap-2 my-10">
        ...
        <Label htmlFor={`prod-showPicture-${categoryId}`}>Show image on menu</Label>
      </div>
      <div className="flex items-center gap-2">
        <input id={`prod-allergen-${categoryId}`} name="allergenWarning" type="checkbox" className="accent-orange-500 cursor-pointer" />
        <Label htmlFor={`prod-allergen-${categoryId}`} className="text-orange-700">Apply allergen warning</Label>
      </div>
      ...
      <div className="flex flex-col gap-2">
        <Button type="submit" size="sm" disabled={pending || submitting} className="w-full cursor-pointer">
          ...
          {(pending || submitting) ? 'Adding...' : 'Add Product'}
        </Button>
        {onClose && (
